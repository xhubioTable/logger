...
This logger is mainly used for unit testing. It stores all the logs in
array by level type. So you can get the Logs after the test and proof
that the right logs where generated.
...
To get all the logs read the property 'logger.entries'. This returns an
hash where for each logLevel the logs are stored.
